...
def _summarize(keywords, base, model, key, retries=6):
    body = json.dumps({
        "model": model,
        "messages": [{"role": "system", "content": _THEME_SYSTEM},
...
def cmd_umap_overview(args) -> int:
...
def cmd_overlay_maps(args) -> int:
    ...
    lo, hi = base.min(0), base.max(0); pad = (hi - lo) * pad_frac
    axL.set_xlim(lo[0] - pad[0], hi[0] + pad[0]); axL.set_ylim(lo[1] - pad[1], hi[1] + pad[1])
    axL.set_xticks([]); axL.set_yticks([])
    for s in axL.spines.values():
        s.set_visible(False)
    if args.map_only:
        axL.patch.set_visible(False)   # drop the white axes rectangle from the tight bbox
    ...
    if axR is not None:
        ...
        for i, c in enumerate(clusters):
            ...
            axR.text(0.04, y - 0.004, _cluster_theme(c), transform=axR.transAxes, fontsize=8.5,
                     color="#222222", va="top")
...
_TBL_COLS = [("k50_cosmos", "Cosmos-Embed1"), ("k50_caption", "Caption (Qwen3-Embedding-8B)"),
...
def _texword(w):
    c = _color(w)
    return latex_escape(w) if c == NEUTRAL else \
        r"\textcolor[HTML]{%s}{\textbf{%s}}" % (c.lstrip("#").upper(), latex_escape(w))
...
def _emit_topics(clustering_dir, out, k, map_width, topics_width, maps_prefix):
    ...
    lines = ["% Overlay figure interior: 3 rows x 2 cols, one row per embedding.",
    ...
    for j, (key, (_, name)) in enumerate(zip(keys, _TBL_COLS)):
        lines.append(r"\begin{minipage}[c]{%.2f\linewidth}\centering"
                     r"\includegraphics[width=\linewidth]{%s%s.png}\end{minipage}\hfill"
                     % (map_width, maps_prefix, key))
        lines.append(r"\begin{minipage}[c]{%.2f\linewidth}\footnotesize\raggedright\textbf{%s}\\[2pt]"
                     % (topics_width, name))
        cells = percol[j]
...
def _emit_distinctive(clustering_dir, out, topn):
    ...
    runs = {name: json.loads((clustering_dir / rid / "cluster_topics.json").read_text())["topics"]
...
def cmd_tables(args) -> int:
    if args.what in ("topics", "both"):
        _emit_topics(args.clustering_dir, args.topics_out, args.k,
                     args.map_width, args.topics_width, args.maps_prefix)
...
def cmd_hierarchical(args) -> int:
    ...
    for r, (flat, hierd, npz, key, name, center) in enumerate(rows):
        ...
        for ci in order[:args.label_k]:
            ...
            axZ.text(lx, ly, "\n".join(textwrap.wrap(_phrase(t[c], 2), 16)),
                     ...
                     bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.85,
                               edgecolor=_PAL[ci % len(_PAL)], linewidth=0.8))
    ...
    for axO, axZ, ftxt, fcol in arrows:
        ...
        con = ConnectionPatch(xyA=start, coordsA=axO.transData, xyB=(0.0, 0.5),
...
def main(argv=None) -> int:
    ...
    p.add_argument("--runs", nargs="+", required=True, help="run-id subdirs to backfill")
    p.add_argument("--base-url", default=os.environ.get("LLM_BASE_URL"))
    p.add_argument("--model", default=os.environ.get("LLM_MODEL"))
    p.add_argument("--workers", type=int, default=2, help="concurrency (gateway rate-limits)")
    p.add_argument("--top-k", type=int, default=10, help="keywords sent per cluster")
    p.add_argument("--topics-name", default="cluster_topics.json",
                   help="topics file per run dir (use hier_topics.json for hierarchical runs)")
    p.set_defaults(func=cmd_themes)

    p = sub.add_parser("umap-overview", help="(pools x embeddings) grid of cluster scatters")
    p.add_argument("--clustering-dir", type=Path, required=True)
    p.add_argument("--fig-runs", type=Path, required=True, help="JSON from prep.py fig-runs")
    p.add_argument("--out", type=Path, default=Path("umap_overview.png"))
    p.set_defaults(func=cmd_umap_overview)

    p = sub.add_parser("overlay-maps", help="one embedding's UMAP with distinct clusters pinned")
    p.add_argument("--run-dir", type=Path, required=True)
    p.add_argument("--out", type=Path, required=True)
    p.add_argument("--title", default="")
    p.add_argument("--k", type=int, default=10, help="clusters shown in the map")
    p.add_argument("--select", choices=["size", "distinct"], default="distinct",
    ...
    p.add_argument("--emit-tex", type=Path, default=None,
    ...
    p = sub.add_parser("tables", help="colour-coded topic table + distinctive-terms table")
    p.add_argument("--clustering-dir", type=Path, required=True)
    p.add_argument("--what", choices=["topics", "distinctive", "both"], default="both")
    p.add_argument("--topics-out", type=Path, default=Path("tables/emb_cluster_topics.tex"))
    p.add_argument("--distinctive-out", type=Path,
                   default=Path("tables/emb_distinctive_terms.tex"))
    p.add_argument("--k", type=int, default=10, help="clusters shown per embedding (topics)")
    p.add_argument("--topn", type=int, default=8, help="distinctive terms per embedding")
    p.add_argument("--maps-prefix", default="figures/overlay_map_",
                   help="prefix for the per-embedding map PNG (<prefix><key>.png)")
    p.add_argument("--map-width", type=float, default=0.45, help="map minipage width (\\linewidth)")
    p.add_argument("--topics-width", type=float, default=0.53,
                   help="topics minipage width (\\linewidth)")
    p.set_defaults(func=cmd_tables)

    p = sub.add_parser("hierarchical", help="drill one branch per embedding into its sub-clusters")
    p.add_argument("--clustering-dir", type=Path, required=True)
