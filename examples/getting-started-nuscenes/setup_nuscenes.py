...
Stages, each toggleable via --skip-* flags:
  ...
  4. Extract Cosmos / Qwen captions / caption embeddings / Florence-2 +
     SigLIP visual embeddings via scripts/extract_*.py.
...
def load_captions_into_db(
    ...
    model_name = f"Qwen2.5-VL-{model_size}B"
...
def materialize_visual_embeddings_index(visual_dir: Path, index_spec: str = "Flat") -> None:
    ...
    store = Florence2SigCLIPEmbeddingStore(str(visual_dir), index_spec=index_spec)
    log.info(
...
def main():
    ...
    parser.add_argument(
        "--qwen-model-size", default=3, type=int, choices=[3, 7, 32, 72],
        help="Qwen2.5-VL size for captioning. 3B fits with the cosmos load on a 24 GiB GPU.",
    )
    parser.add_argument(
